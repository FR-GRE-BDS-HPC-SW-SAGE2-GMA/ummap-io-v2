...
TEST(TestMmapDriver, anonymous_offset)
{
	MmapDriver driver;
	size_t size = 4* UMMAP_PAGE_SIZE;
	char * ptr = (char*)driver.directMmap(size, 4096, true, true);
	memset(ptr, 10, size);
...
TEST(TestMmapDriver, anonymous_not_aligned)
{
	...
	ASSERT_DEATH(driver.directMmap(size, 1024, true, true), "aligned");
	ASSERT_DEATH(driver.directMmap(size+1024, 0, true, true), "aligned");
}
...
TEST(TestMmapDriver, fileMapping_no_offset)
{
	...
	char * ptr = (char*)driver.directMmap(size, offset, true, true);
	for (size_t i = 0 ; i < size ; i++)
...
TEST(TestMmapDriver, fileMapping_aligned_offset)
{
	...
	MmapDriver driver(fileno(fp));
	char * ptr = (char*)driver.directMmap(fsize, offset, true, true);
